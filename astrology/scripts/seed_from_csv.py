...
Usage: python -m astrology.scripts.seed_from_csv [--overwrite]
"""
import argparse
import asyncio
import csv
...
from astrology.logic.interpretations.lexicons import (
...
def _data_dir() -> Path:
    ...
    p = Path(__file__).resolve().parents[2] / "data" / "new"
    if p.exists():
        return p
...
async def load_from_csv(session: AsyncSession, *, overwrite: bool = False) -> None:
    ...
    # Build lookup maps (reference tables must already exist from astrology.scripts.seed)
...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        # Ensure reference data exists (run astrology.scripts.seed first if needed)
        planets = (await session.execute(select(Planet))).scalars().all()
        if not planets:
            print("Reference data missing. Run 'python -m astrology.scripts.seed' first.")
            return
